...
public class LinkedListController {
	...
	public void removeFirst() {
		if(list.isEmpty()) {
			cb.onStatusError("List is empty");
			return;
		}
		resetIterator();
	...
	public void removeValue(int value) {
		...
		if(found) {
			cb.onStatusOk("remove(" + value + ") — removed");
			cb.onListChanged();
		}
	...
	public void iterNext() {
		if(iterator == null || !iterator.hasNext()) {
			cb.onStatusError("No next element");
			return;
		}
		int v = iterator.next();
	...
	public void iterPrevious() {
		if(iterator == null || !iterator.hasPrevious()) {
			cb.onStatusError("No previous element");
			return;
		}
	...
	public void iterSet(int value) {
		if(iterator == null || iteratorIndex < 0) {
			cb.onStatusError("Call next() or previous() before set()");
			return;
		}
		iterator.set(value);
	...
	public void iterRemove() {
		if(iterator == null || iteratorIndex < 0) {
			cb.onStatusError("Call next() or previous() before remove()");
			return;
		}
		iterator.remove();
